...
module MysteryShopper
	class DetailDefinitionEntry
		...
		def self.create(name, details)
			if !!details[:data_type] && details[:data_type].class.method_defined?(:to_s) && MysteryShopper.const_defined?("#{details[:data_type].to_s.split('_').map(&:capitalize).join}DetailDefinitionEntry")
				dde = MysteryShopper.const_get("#{details[:data_type].to_s.split('_').map(&:capitalize).join}DetailDefinitionEntry").new(name, details)
			else
